# Flutter & Dart Practice CLI

A command-line tool built using Dart to practice core concepts, command-line arguments, and external API interactions.

---

## Features & Commands
- `version` - Displays the current version of the CLI tool.
- `help` - Lists all available commands.
- `search <query>` - Searches Wikipedia for the specified topic.

---

## Learning Log & Progress

### Day 1: Setup & Basic Commands
- Set up the repository structure and linked it with GitHub.
- Parsed command-line arguments passed via `main(List<String> arguments)`.
- Created interactive prompt handling using `stdin.readLineSync()`.